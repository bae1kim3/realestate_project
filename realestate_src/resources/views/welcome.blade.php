<x-app-layout>
            ...
            @if (session('seller_license'))
                <div class="sm:fixed sm:top-0 sm:right-0 p-6 text-right z-10">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex justify-center">
                        ...
                        <a href="{{ url('/dashboard') }}" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-white dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">매물올리기</a>
                    </div>
                        @else
                        <a href="{{ route('login') }}" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">로그인</a>

                        @if (Auth::user())
                            <a href="{{ route('register') }}" class="ml-4 font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">회원가입</a>
                        @endif
                    @endauth
                </div>
            @endif
  ...
  <div class="flex items-start gap-8">
    ...
    <div class="accordion">
        ...
        <div class="accordion-item">
          <div class="accordion-title font-bold cursor-pointer dark:text-white dark:text-white">세인트동물병원</div>
          <div class="accordion-content">
