...
<style>
    .scroll-item {
        overflow-x: auto;
        white-space: nowrap;
        width: 100%; /* 스크롤 컨테이너의 너비를 100%로 설정 */
    }
...
<script>
    var loadingPhotos = false; // 추가된 변수
    var photosLoaded = 0; // 추가된 변수

    function loadMorePhotos() {
        if (loadingPhotos) return; // 로딩 중인 경우 중복 로드 방지
        loadingPhotos = true; // 로딩 상태로 변경

        var lastPhotoId = document.querySelector('.photo-item:last-child').getAttribute('data-photo-id');
